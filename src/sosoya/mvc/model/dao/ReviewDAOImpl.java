...
public class ReviewDAOImpl implements ReviewDAO {
	...
	public int insertReview(ReviewVO reviewVO) throws SQLException {
		...
		try {
			...
			// 3번째 ?값이 orderDetailCode로 들어가야 된다.
			ps.setInt(3, reviewVO.getOrdersDetailsCode());
			
			ps.setString(4, reviewVO.getReviewTitle());
			ps.setString(5, reviewVO.getReviewContent());
			ps.setInt(6, reviewVO.getReviewGrade());
			...
			if(result == 0) {
				...
				if(goodsVO == null) {
					...
					// 업데이트 전 상품 평점평균
					float beforeGoodsTotalValue = 0.0f;
					
					// 업데이트 후 상품 평점평균
					float afterGoodsTotalAvg = 0.0f;
					
					// 처음 처음뷰가 등록될 때 count가 1이다.
					if(reviewCount == 1) {
						afterGoodsTotalAvg = reviewVO.getReviewGrade();
					} else {
						// 업데이트 전 상품의 총평점을 가져온다.
						// System.out.println(goodsVO.getGoodsGradeAvg());
						beforeGoodsTotalValue = goodsVO.getGoodsGradeAvg() * (reviewCount - 1);
						
						// 사용자가 입력한 평점을 더하여 최종적으로 count + 1해서 나누어준 최신 평점평균을 구한다.
						afterGoodsTotalAvg = (beforeGoodsTotalValue + reviewVO.getReviewGrade()) / reviewCount;
						
						// 소수점 둘째자리에서 반올림해서 첫째자리까지 나타낸다. 
						afterGoodsTotalAvg = Math.round(afterGoodsTotalAvg*10)/10.0f;
					}
					
					result = goodsDao.updateGoodsAvg(reviewVO.getGoodsCode(), afterGoodsTotalAvg, con);
					if(result == 0) {
						con.rollback();
						throw new SQLException("goods테이블의 평점평균이 업데이트 되지 않았습니다.");
					}
				}
			}	
		} finally {
	...
	public ReviewVO selectByIdOrderDetailCode(String memberId, int ordersDetailsCode) {
		...
		PreparedStatement ps = null;
		ResultSet rs = null;
		String sql = sosoyaSql.getProperty("REVIEW.SELECTBYIDORDERDETAILCODE");
		...
		try {
			...
			ps.setInt(2, ordersDetailsCode);
			
			rs = ps.executeQuery();
			if(rs.next()) {
				reviewVoCheck = new ReviewVO();
				reviewVoCheck.setReviewCode(rs.getInt("REVIEW_CODE"));
			}
		} catch (Exception e) {
